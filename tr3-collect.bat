...
::				5 = Acquire NTFS artifacts only
::				6 = Acquire Memory Forensic Image, Volatile, and Non-Volatile Data 
::
...
cls
	...
	if %selection% == 5 echo %DATE% %TIME% - Selection was made to acquire NTFS artifacts for %COMPUTERNAME% >> %c_drive%:\Data-%case%\Collection.log
	if %selection% == 6 echo %DATE% %TIME% - Selection was made to acquire memory forensic image, volatile, and non-volatile data %COMPUTERNAME% >> %c_drive%:\Data-%case%\Collection.log
	goto :main
:: --------------------------------------------------------------------------------------------------------------------------
...
::
	...
	if %selection% == 5 goto :acquire_ntfs
	if %selection% == 6 goto :acquire_memory
:: ****************************************************************************************************************************
...
:acquire_memory
	...
	if %selection% == 6 goto :acquire_volatile
	goto :exit
:acquire_volatile
	...
	:: Collecting process information
		...
		:: cprocess.exe  /stext reference: Malware Forensics page 38
			...
			tools\cprocess.exe  /stext %vol_outpath%\ProcessInfo_2_process-to-user-mapping.txt
		...
		:: cprocess.exe  /stab reference: Malware Forensics page 38
			...
			tools\cprocess.exe  /stab %vol_outpath%\ProcessInfo_2_process-to-user-mapping_tab.csv
		...
		:: pslist.exe -t reference: Malware Forensics page 40 or WFA page 26
			...
			tools\pslist.exe -t /accepteula >> %vol_outpath%\ProcessInfo_5_child-processes.txt
		...
		:: handle.exe reference: Malware Forensics page 42 or WFA page 27
			...
			tools\handle.exe /accepteula >> %vol_outpath%\ProcessInfo_6_processe-file-handles.txt
		...
		:: listdlls.exe reference: Malware Forensics page 44 or WFA page 26
			...
			tools\listdlls.exe /accepteula >> %vol_outpath%\ProcessInfo_7_processe-dependencies.txt
	...
	:: Collecting opened files information
		...
		if %arch% == 64 (
			echo %DATE% %TIME% - Running tools\openedfilesview.x64.exe /stext on %COMPUTERNAME% to obtain the open files on the computer
			echo %DATE% %TIME% - Running tools\openedfilesview.x64.exe /stext on %COMPUTERNAME% to obtain the open files on the computer >> %c_drive%:\Data-%case%\Collection.log	
			START /WAIT tools\openedfilesview.x64.exe /stext %vol_outpath%\OpenedFilesInfo_1_opened-files.txt
		}
	...
	if %selection% == 6 goto :acquire_nonvolatile
	goto :exit
:acquire_nonvolatile
	...
	:: Collecting the System's Registry Hives
		...
		if  NOT %os% == legacy (
			...
			if %arch% == 32 (
				...
				tools\RawCopy.exe %WINDIR%\System32\config\RegBack\DEFAULT %nonvol_outpath%\registry\RegBack
			)
			if %arch% == 64 (
	...
	:: Collecting log files
		...
		if %os% == legacy (
			echo %DATE% %TIME% - Running windir%\System32\xcopy.exe on %COMPUTERNAME% to collect the McAfee log folder
			echo %DATE% %TIME% - Running windir%\System32\xcopy.exe on %COMPUTERNAME% to collect the McAfee log folder >> %c_drive%:\Data-%case%\Collection.log
			%windir%\System32\xcopy.exe "%SYSTEMDRIVE%\Documents and Settings\All Users\Application Data\McAfee\DesktopProtection" %nonvol_outpath%\logs\McAfee\DesktopProtection /e /i
			echo %DATE% %TIME% - Running windir%\System32\xcopy.exe on %COMPUTERNAME% to collect the McAfee Quarantine folder
			echo %DATE% %TIME% - Running windir%\System32\xcopy.exe on %COMPUTERNAME% to collect the McAfee Quarantine folder >> %c_drive%:\Data-%case%\Collection.log
			%windir%\System32\xcopy.exe "%SYSTEMDRIVE%\Quarantine" %nonvol_outpath%\logs\McAfee\Quarantine /e /i
			) else (
				echo %DATE% %TIME% - Running windir%\System32\xcopy.exe on %COMPUTERNAME% to collect the McAfee log folder
				echo %DATE% %TIME% - Running windir%\System32\xcopy.exe on %COMPUTERNAME% to collect the McAfee log folder >> %c_drive%:\Data-%case%\Collection.log
				%windir%\System32\xcopy.exe %SYSTEMDRIVE%\ProgramData\McAfee\DesktopProtection %nonvol_outpath%\logs\McAfee\DesktopProtection /e /i
				echo %DATE% %TIME% - Running windir%\System32\xcopy.exe on %COMPUTERNAME% to collect the McAfee Quarantine folder
				echo %DATE% %TIME% - Running windir%\System32\xcopy.exe on %COMPUTERNAME% to collect the McAfee Quarantine folder >> %c_drive%:\Data-%case%\Collection.log
				%windir%\System32\xcopy.exe %SYSTEMDRIVE%\Quarantine %nonvol_outpath%\logs\McAfee\Quarantine /e /i
		)
	...
	echo:
	echo %DATE% %TIME% - Completed acquiring %COMPUTERNAME%'s non-volatile data >> %c_drive%:\Data-%case%\Collection.log
	echo:
	:: Exiting the acquire_nonvolatile function
	if %selection% == 3 goto :acquire_ntfs
	if %selection% == 4 goto :acquire_ntfs
	if %selection% == 6 goto :acquire_ntfs
	goto :exit
:acquire_ntfs
	:: The acquire_ntfs function obtains NTFS artifacts from the system to help with triage
	:: Creating output directory
	if not exist %c_drive%:\Data-%case%\%computername%-%timestamp%\nonvolatile-data (
		echo %DATE% %TIME% - Running tools\mkdir.exe on %COMPUTERNAME% to create directory to store the non-volatile data
		echo %DATE% %TIME% - Running tools\mkdir.exe on %COMPUTERNAME% to create directory to store the non-volatile data >> %c_drive%:\Data-%case%\Collection.log
	...
	:: Collecting NTFS artifacts
		...
		:: Collecting the $UsnJrnl record
		echo %DATE% %TIME% - Running tools\ExtractUsnJrnl.exe %SYSTEMDRIVE% on %COMPUTERNAME% to extract the $UsnJrnl
		echo %DATE% %TIME% - Running tools\ExtractUsnJrnl.exe %SYSTEMDRIVE% on %COMPUTERNAME% to extract the $UsnJrnl  >> %c_drive%:\Data-%case%\Collection.log
		if %arch% == 32 (tools\ExtractUsnJrnl.exe %SYSTEMDRIVE% %nonvol_outpath%\ntfs) else (tools\ExtractUsnJrnl64.exe %SYSTEMDRIVE% %nonvol_outpath%\ntfs)
	:: Exiting the acquire_ntfs function
	goto :exit
:: --------------------------------------------------------------------------------------------------------------------------
